...
export function Modal({ width = 'md', className, children, isOpen = false, onClose }: ModalProps) {
...
Modal.Header = function ModalHeader({
  ...
  return <div className={cn(className, 'px-6 pt-6 pb-4 text-2xl')}>{children}</div>
}
...
Modal.Body = function ModalBody({
  ...
  return <div className={cn(className, 'px-6')}>{children}</div>
}
...
Modal.Footer = function ModalFooter({
  ...
  return <div className={cn(className, 'mt-auto p-6 flex gap-6 justify-end')}>{children}</div>
}
